#include "ism330dhcx/ISM330DHCX.hpp"
...
float Device::gyro_sensitivity() const noexcept
{
    ...
    switch (m_cfg.gyro_range) {
        ...
        case GyroRange::dps_4000: return 0.140000f;
...
bool Device::initialize(Transport t, Config cfg)
{
    ...
    // Enable accel + gyro full-scale range access
    if (!write_reg(reg::CTRL9_XL, 0x02)) return false;
    ...
    // CTRL7_G: HP filter off, FS_4000 path enable if needed (handled via FS bits already)
