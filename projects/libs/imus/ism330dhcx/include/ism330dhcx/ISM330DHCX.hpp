...
enum class AccelRange : uint8_t {
    g2  = 0x00,  ///< ±2 g  — 0.061 mg/LSB
    g16 = 0x04,  ///< ±16 g — 0.488 mg/LSB
    g4  = 0x08,  ///< ±4 g  — 0.122 mg/LSB
    g8  = 0x0C,  ///< ±8 g  — 0.244 mg/LSB
};
...
/// FS_125 (bit 1) and FS_4000 (bit 0) are special single-bit overrides.
enum class GyroRange : uint8_t {
    ...
    dps_4000 = 0x01,  ///< ±4000 dps — 140.0 mdps/LSB (ISM330DHCX only, FS_4000 bit)
};
...
/// Sensor tag identifying the data type in each FIFO word.
enum class FifoTag : uint8_t {
    empty         = 0x00,
    gyro_nc       = 0x01,  ///< Gyroscope, no compression
    accel_nc      = 0x02,  ///< Accelerometer, no compression
    temperature   = 0x03,
    timestamp     = 0x04,
    other         = 0xFF,
};

/// One 7-byte FIFO word (1 tag + 6 data bytes).
struct FifoEntry {
    FifoTag tag{};
    uint8_t data[6]{};
};
...
// ---------------------------------------------------------------------------
// ISM330DHCX 6-axis IMU driver (3-axis accel ±2/4/8/16 g, 3-axis gyro ±125..4000 dps)
//
// Usage:
//   ism330dhcx::Device imu;
...
public:
    ...
    static constexpr uint8_t kWhoAmIExpected = 0x6B;
    ...
    /// Number of unread words currently in the FIFO (0 if FIFO disabled).
    [[nodiscard]] uint16_t fifo_count() const;

    /// Pop one word from the FIFO.  Returns false on comm error or empty FIFO.
