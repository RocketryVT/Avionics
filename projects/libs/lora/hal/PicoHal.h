// PicoHal.h — RadioLib hardware abstraction layer for the Raspberry Pi Pico SDK.
//
// Implements RadioLibHal so any RadioLib radio class (SX1276, RF69, SX126x, …)
// can run on RP2040 / RP2350 without Arduino.
#pragma once
...
public:
    PicoHal( spi_inst_t* spi, uint8_t sck, uint8_t mosi, uint8_t miso )
        : RadioLibHal( GPIO_IN, GPIO_OUT, 0, 1,
                       GPIO_IRQ_EDGE_RISE, GPIO_IRQ_EDGE_FALL ),
          _spi( spi ), _sck( sck ), _mosi( mosi ), _miso( miso )
    {}

    void init() override {
        spi_init( _spi, 1'000'000 );
    ...
    void attachInterrupt( uint32_t pin, void (*func)(void), uint32_t mode ) override {
        gpio_set_irq_enabled_with_callback( pin, mode, true,
                                            reinterpret_cast<gpio_irq_callback_t>( func ) );
    }
