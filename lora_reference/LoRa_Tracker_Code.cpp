...
int main( void )
{
    ...
    // needed before "set tx" is called
    status = lr11xx_radio_set_pkt_type( &ctx, LR11XX_RADIO_PKT_TYPE_LORA );
    printf( "set_pkt_type=%d\n", status );
    check_lr11xx( &ctx, LED_PIN, status, "set_pkt_type failed" );

    // Set LoRa sync word
    status = lr11xx_radio_set_lora_sync_word( &ctx, LORA_SYNC_WORD );
    printf( "set_lora_sync_word=0x%02X status=%d\n", LORA_SYNC_WORD, status );
    check_lr11xx( &ctx, LED_PIN, status, "set_lora_sync_word failed" );

    //status = lr11xx_radio_set_rf_freq( &ctx, 915000000 );
    ...
    status = lr11xx_radio_set_lora_mod_params( &ctx, &lora_mod_params );
    printf( "set_lora_mod_params=%d\n", status );
    check_lr11xx( &ctx, LED_PIN, status, "set_lora_mod_params failed" );
                ...
                // Optional: set GFSK sync word (8 bytes)
                const uint8_t gfsk_sync_word[LR11XX_RADIO_GFSK_SYNC_WORD_LENGTH] = LR11XX_RADIO_GFSK_SYNC_WORD_DEFAULT;
                ...
                lr11xx_radio_mod_params_gfsk_t gfsk_mod_params = {
                    ...
                    .bw_dsb_param = LR11XX_RADIO_GFSK_BW_78200,
                    .fdev_in_hz  = 25000,
                };
        ...
        uint32_t toa_ms = lr11xx_radio_get_lora_time_on_air_in_ms(&lora_pkt_params, &lora_mod_params);
        ...
        {
            printf("Packet sent, time on air=%u ms counter=%u\n", toa_ms, tx_counter);
            gpio_xor_mask( 1u << LED_PIN );
        ...
        while (true)
        {
            ...
            if (to_ms_since_boot(get_absolute_time()) - start > (int)(toa_ms + 5000)) // safety timeout
            {
